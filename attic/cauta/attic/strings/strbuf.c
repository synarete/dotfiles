...
static void str_memcopy(char *s1, const char *s2, size_t n)
{
	memcpy(s1, s2, n);
}

static void str_memmove(char *s1, const char *s2, size_t n)
{
	memmove(s1, s2, n);
}
 ...
 */
static size_t
str_insert_no_overlap(char *buf, size_t sz, size_t n1,
 ...
 */
static size_t
str_insert_with_overlap(char *buf, size_t sz, size_t n1,
                        const char *s, size_t n2)
{
                      ...
                      size_t pos, const char *s, size_t n)
{
	 ...
	 */
	m = str_insert(buf + j, rem, k, s, n);
	return j + m;
}

/*. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .*/

size_t str_insert_chr(char *buf, size_t sz, size_t n1,
                      size_t n2, char c)
{
	size_t k, m;
                       ...
                       size_t pos, size_t n, char c)
{
	 ...
	 */
	m = str_insert_chr(buf + j, rem, k, n, c);
	return j + m;
}
                   ...
                   const char *s, size_t n2)
{
	...
	if (n1 < n2) {
		...
		if (s < buf) {
			k = n1;
			m = str_insert(buf + k, sz - k, len - k, s + k, n2 - k);
			str_copy(buf, s, k);
		} else {
			k = n1;
			str_copy(buf, s, n1);
                       ...
                       size_t pos, size_t cnt, const char *s, size_t n)
{
	 ...
	 * insufficient buffer capacity.
	 */
	m = str_replace(buf + pos, wr - pos, sz - pos, r, s, n);
                       ...
                       size_t n1, size_t n2, char c)
{
	...
	if (n1 < n2) {
		...
		k = n1;
		str_fill(buf, k, c);
		m = str_insert_chr(buf + k, sz - k, len - k, n2 - k, c);
	} else {
		/*
                        ...
                        size_t pos, size_t cnt, size_t n, char c)
{
	 ...
	 */
	m = str_replace_chr(buf + pos, wr - pos, sz - pos, r, n, c);
