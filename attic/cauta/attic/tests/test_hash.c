...
#define assert_eq(a, b) cac_assert((a) == (b))
...
static void test_siphash(void)
{
	...
	for (i = 0; i < 64; i++) {
		...
		cac_hash_siphash64(key, msg, i, &res);
		assert_eq(tst, res);
	}
}
...
static void test_murmur3(void)
{
	...
	str = "";
	cac_hash_murmur3(str, strlen(str), 0, res);
	assert_eq(res[0], 0);
	assert_eq(res[1], 0);

	cac_hash_murmur3(str, strlen(str), 1, res);
 ...
 */
static void test_cubehash(void)
{
	const char *str;
	uint64_t res[8];

	str = "Hello";
	cac_hash_cubehash512(str, strlen(str), res);
