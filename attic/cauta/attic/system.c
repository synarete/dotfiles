...
int sys_getdent(int fd, loff_t off, sys_dirent_t *dent)
{
	long rc, dt, nread;
	size_t len;
	char buf[1024];
...
int sys_create(const char *path, mode_t mode, int *fd)
{
	int rc;
...
int sys_open_rdonly(const char *path, int *fd)
{
	return sys_open(path, O_RDONLY, 0, fd);
}
...
int sys_realpath(const char *path, char **resp)
{
	int rc = 0;
	char *real;
...
char *sys_makepath(const char *base, ...)
{
	va_list ap;
	char *path;
	const char *comp;
	...
	const size_t nelem = CAC_NELEMS(comps);
...
char *sys_joinpath(const char *base, const char *comp)
{
	return sys_makepath(base, comp, NULL);
}

void sys_freepath(char *path)
{
	free(path);
}

void sys_freeppath(char **ppath)
{
	sys_freepath(*ppath);
	*ppath = NULL;
}
...
int sys_read(int fd, void *buf, size_t cnt, size_t *nrd)
{
	int rc;
	ssize_t res;
...
int sys_pread(int fd, void *buf, size_t cnt, loff_t off, size_t *nrd)
{
	int rc;
	ssize_t res;
...
int sys_write(int fd, const void *buf, size_t cnt, size_t *nwr)
{
	int rc;
	ssize_t res;
...
int sys_pwrite(int fd, const void *buf, size_t cnt, loff_t off, size_t *nwr)
{
	int rc;
	ssize_t res;
...
int sys_lseek(int fd, loff_t off, int whence, loff_t *pos)
{
	int rc;
	loff_t res;
...
int sys_fallocate(int fd, int mode, loff_t off, loff_t len)
{
	int rc;
...
int sys_truncate(const char *path, loff_t len)
{
	int rc;
...
int sys_ftruncate(int fd, loff_t len)
{
	int rc;
...
int sys_readlink(const char *path, char *buf, size_t bsz, size_t *nwr)
{
	int rc = 0;
	ssize_t res;
...
int sys_statdev(const char *path, struct stat *st, dev_t *dev)
{
	int rc;
	struct stat buf;
...
int sys_statdir(const char *path, struct stat *st)
{
	int rc;
	struct stat buf;
...
int sys_statedir(const char *path, struct stat *st)
{
	int rc;
	struct stat buf;
...
int sys_statreg(const char *path, struct stat *st)
{
	int rc;
	struct stat buf;
...
int sys_statblk(const char *path, struct stat *st)
{
	int rc;
	struct stat buf;
...
int sys_statvol(const char *path, struct stat *st, mode_t mode)
{
	int rc;
	struct stat buf;
...
int sys_statsz(const char *path, struct stat *st, loff_t *sz)
{
	int rc;
	struct stat buf;
...
int sys_fstatvfs(int fd, struct statvfs *stv)
{
	int rc;
	struct statvfs buf;
...
int sys_statvfs(const char *path, struct statvfs *stv)
{
	int rc;
	struct statvfs buf;
...
int sys_mmap(int fd, loff_t off, size_t len,
