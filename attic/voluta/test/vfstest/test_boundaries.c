 ...
 */
static void test_boundaries_(struct vt_env *vt_env, loff_t boff)
{
	int fd;
	uint8_t byte;
	uint64_t val1, val2;
...
static void test_boundaries_arr_(struct vt_env *vt_env,
				 const loff_t *arr, size_t cnt)
{
	for (size_t i = 0; i < cnt; ++i) {
		test_boundaries_(vt_env, arr[i]);
	}
}

static void test_boundaries_write_read(struct vt_env *vt_env)
{
	const loff_t offs[] = {
	...
	test_boundaries_arr_(vt_env, offs, VT_ARRAY_SIZE(offs));
}

static void test_boundaries_tree_levels(struct vt_env *vt_env)
{
	const loff_t offs[] = {
		VT_BK_SIZE,
