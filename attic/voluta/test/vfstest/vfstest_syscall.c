...
void vt_utimes(const char *filename, const struct timeval times[2])
{
...
void vt_pwrite(int fd, const void *buf, size_t cnt,
...
void vt_listxattr(const char *path, char *list,
		  size_t size, size_t *out)
{
	expect_ok(voluta_sys_listxattr(path, list, size, out));
}

void vt_llistxattr(const char *path, char *list,
		   size_t size, size_t *out)
{
...
void vt_getdents(int fd, void *buf, size_t bsz, struct dirent64 *dents,
