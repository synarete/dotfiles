 ...
 */
static void test_fsync_reg(struct vt_env *vt_env, loff_t base_off,
			   size_t bsz, loff_t step, size_t cnt)
{
	int fd;
	loff_t off;
	char *buf1, *buf2 = vt_new_buf_rands(vt_env, bsz);
...
static void test_fsync_reg_aligned(struct vt_env *vt_env)
{
	test_fsync_reg(vt_env, 0, VT_UKILO, VT_UMEGA, 64);
	test_fsync_reg(vt_env, VT_UMEGA, VT_BK_SIZE, VT_UGIGA,
		       64);
}

static void test_fsync_reg_unaligned(struct vt_env *vt_env)
{
	test_fsync_reg(vt_env, 1, VT_UKILO - 1, VT_UMEGA + 1, 64);
	test_fsync_reg(vt_env, VT_UMEGA - 1,
 ...
 */
static void test_fsync_dir_nent(struct vt_env *vt_env, size_t cnt)
{
	int fd, dfd;
	...
	for (size_t i = 0; i < cnt; ++i) {
		path2 = vt_new_pathf(vt_env, path1, "%08x", i + 1);
	...
	for (size_t j = 0; j < cnt; ++j) {
		path2 = vt_new_pathf(vt_env, path1, "%08x", j + 1);
		vt_unlink(path2);
		vt_fsync(dfd);
	}
	vt_close(dfd);
...
static void test_fsync_dir(struct vt_env *vt_env)
{
	test_fsync_dir_nent(vt_env, 1024);
	test_fsync_dir_nent(vt_env, 4096);
}
 ...
 */
static void test_fsync_syncfs(struct vt_env *vt_env)
{
	int fd;
	const char *path = vt_new_path_unique(vt_env);
