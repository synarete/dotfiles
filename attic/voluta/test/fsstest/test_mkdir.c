 ...
 */
static void test_rmdir_openat(struct voluta_t_ctx *t_ctx)
{
	int dfd1;
	int dfd2;
	struct stat st;
	...
	const char *path1 = voluta_t_new_path_unique(t_ctx);
	const char *path2 = voluta_t_new_path_nested(t_ctx, path1, name);

	voluta_t_mkdir(path1, 0700);
	voluta_t_open(path1, O_DIRECTORY | O_RDONLY, 0, &dfd1);
	voluta_t_mkdirat(dfd1, name, 0700);
	voluta_t_openat(dfd1, name, O_DIRECTORY | O_RDONLY, 0, &dfd2);
	voluta_t_fstat(dfd1, &st);
	voluta_t_expect_true(S_ISDIR(st.st_mode));
	voluta_t_fstat(dfd2, &st);
	voluta_t_expect_true(S_ISDIR(st.st_mode));
	voluta_t_expect_eq(st.st_nlink, 2);
	voluta_t_rmdir(path2);
	voluta_t_fstat(dfd2, &st);
	voluta_t_expect_true(S_ISDIR(st.st_mode));
	voluta_t_expect_le(st.st_nlink, 1); /* TODO: why not eq 1 ? */
	voluta_t_rmdir(path1);
	voluta_t_close(dfd1);
	voluta_t_close(dfd2);
}
...
static const struct voluta_t_tdef t_local_tests[] = {
	...
	VOLUTA_T_DEFTEST(test_rmdir_openat),
};
