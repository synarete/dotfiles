...
void voluta_t_readn(int fd, void *buf, size_t cnt)
{
	size_t nrd_cur, nrd = 0;
	char *ptr;

	while (nrd < cnt) {
		ptr = (char *)buf + nrd;
		nrd_cur = 0;
		voluta_t_read(fd, ptr, cnt - nrd, &nrd_cur);
		if (!nrd_cur) {
			break;
		}
		nrd += nrd_cur;
...
void voluta_t_preadn(int fd, void *buf, size_t cnt, loff_t offset)
{
	size_t nrd_cur, nrd = 0;
	loff_t off;
	char *ptr;
	...
	while (nrd < cnt) {
		...
		nrd_cur = 0;
		voluta_t_pread(fd, ptr, cnt - nrd, off, &nrd_cur);
		if (!nrd_cur) {
			break;
		}
		nrd += nrd_cur;
...
void voluta_t_writen(int fd, const void *buf, size_t cnt)
{
	size_t nwr_cur, nwr = 0;
	char *ptr;

	while (nwr < cnt) {
		ptr = (char *)buf + nwr;
		nwr_cur = 0;
		voluta_t_write(fd, ptr, cnt - nwr, &nwr_cur);
		if (!nwr_cur) {
			break;
		}
		nwr += nwr_cur;
...
void voluta_t_pwriten(int fd, const void *buf, size_t cnt, loff_t offset)
{
	size_t nwr_cur, nwr = 0;
	loff_t off;
	char *ptr;
	...
	while (nwr < cnt) {
		...
		nwr_cur = 0;
		voluta_t_pwrite(fd, ptr, cnt - nwr, off, &nwr_cur);
		if (!nwr_cur) {
			break;
		}
		nwr += nwr_cur;
