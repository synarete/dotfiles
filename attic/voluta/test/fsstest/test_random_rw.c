			 ...
			 size_t len, size_t cnt, int rewrite)
{
	...
	for (i = 0; i < nitr; ++i) {
		for (j = 0; j < cnt; ++j) {
			...
			voluta_t_pwriten(fd, buf1, len, pos);
		}
		for (j = 0; j < cnt; ++j) {
			idx = pseq[j];
			voluta_assert_lt(idx, cnt);
			...
			seed = i + j + (size_t)pos;
			buf1 = voluta_t_new_buf_nums(t_ctx, seed, len);
			voluta_t_preadn(fd, buf2, len, pos);
			voluta_t_expect_eqm(buf1, buf2, len);
		}
	}
	voluta_t_close(fd);
...
static void test_random_aligned_blk(struct voluta_t_ctx *t_ctx, size_t cnt)
{
	loff_t from;
	const size_t len = BK_SIZE;
	...
	from = (loff_t)(UGIGA + BK_SIZE);
	test_random_io(t_ctx, from, len, cnt);
	from = (loff_t)(UGIGA - (len * cnt));
	test_random_io(t_ctx, from, len, cnt);
	from = (loff_t)((UGIGA) / 2);
	test_random_io(t_ctx, from, len, cnt);
	from = (loff_t)(UTERA - (len * cnt));
	test_random_io(t_ctx, from, len, cnt);
}
