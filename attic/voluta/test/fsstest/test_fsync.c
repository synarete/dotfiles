...
{
	test_fsync_reg(t_ctx, 0, UKILO, UMEGA, 64);
	test_fsync_reg(t_ctx, UMEGA, BK_SIZE, UGIGA,
		       64);
}

static void test_fsync_reg_unaligned(struct voluta_t_ctx *t_ctx)
{
	test_fsync_reg(t_ctx, 1, UKILO - 1, UMEGA + 1, 64);
	test_fsync_reg(t_ctx, UMEGA - 1, 3 * UKILO + 1, UGIGA - 1, 64);
}
...
const struct voluta_t_tests voluta_t_test_fsync =
