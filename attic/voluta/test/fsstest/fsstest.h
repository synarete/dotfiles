...
#ifndef VOLUTA_FSSTEST_H_
#define VOLUTA_FSSTEST_H_
...
#define voluta_t_expect_dir(m)          voluta_t_expect_true(S_ISDIR(m))
#define voluta_t_expect_reg(m)          voluta_t_expect_true(S_ISREG(m))
#define voluta_t_expect_lnk(m)          voluta_t_expect_true(S_ISLNK(m))
...
enum voluta_t_flags {
	VOLUTA_T_IGNORE             = (1 << 1),
	VOLUTA_T_NORMAL             = (1 << 2),
	VOLUTA_T_POSIX_EXTRA        = (1 << 3),
	VOLUTA_T_STAVFS             = (1 << 4),
	VOLUTA_T_IO_EXTRA           = (1 << 5),
...
extern const struct voluta_t_tests voluta_t_test_fiemap;
...
void voluta_t_mkdirat(int dirfd, const char *pathname, mode_t mode);
...
void voluta_t_setxattr(const char *, const char *, const void *, size_t, int);
...
void voluta_t_fiemap(int fd, struct fiemap *fm);

/* Complex wrappers */
void voluta_t_readn(int, void *, size_t);
void voluta_t_preadn(int, void *, size_t, loff_t);
void voluta_t_writen(int, const void *, size_t);
void voluta_t_pwriten(int, const void *, size_t, loff_t);

#endif /* VOLUTA_FSSTEST_H_ */
