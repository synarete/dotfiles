...
struct voluta_ut_readdir_ctx *voluta_ut_new_readdir_ctx(
...
static void ut_dir_iter_simple(struct voluta_ut_ctx *ut_ctx)
{
	int err;
	size_t i, count;
	ino_t dino, root_ino = VOLUTA_INO_ROOT;
	const char *name;
	const char *dirname = UT_NAME;
	struct stat st;
	struct voluta_ut_readdir_ctx *readdir_ctx;

	err = voluta_ut_mkdir(ut_ctx, root_ino, dirname, 0700, &st);
	ut_assert_ok(err);
	...
	count = VOLUTA_ARRAY_SIZE(readdir_ctx->dents) - 2;
	for (i = 0; i < count; ++i) {
	...
	err = voluta_ut_rmdir(ut_ctx, root_ino, dirname);
	ut_assert_ok(err);
}
...
static void ut_dir_iter_links_(struct voluta_ut_ctx *ut_ctx, size_t count)
{
	...
	loff_t doff;
	size_t i, ndents;
	ino_t ino, dino, dino2, root_ino = VOLUTA_INO_ROOT;
...
static void ut_dir_iter_links(struct voluta_ut_ctx *ut_ctx)
{
	...
	ut_dir_iter_links_(ut_ctx, 20000);
}
...
static void ut_dir_iter_unlink_(struct voluta_ut_ctx *ut_ctx, size_t count)
{
	...
	err = voluta_ut_lookup(ut_ctx, root_ino, dname, &st);
	ut_assert_err(err, -ENOENT);
}
...
const struct voluta_ut_tests voluta_ut_dir_iter_tests =
