...
void voluta_ut_make_dir(struct voluta_ut_ctx *ut_ctx, ino_t parent_ino,
...
{
	int err, flags = XATTR_CREATE;

	err = voluta_ut_setxattr(ut_ctx, ino, kv->name,
				 kv->value, kv->size, flags);
	ut_assert_ok(err);
