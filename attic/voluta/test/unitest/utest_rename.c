...
static void ut_rename_move_multi(struct voluta_ut_ctx *ut_ctx)
{
	size_t i, cnt = 4096;
	const size_t name_max = VOLUTA_NAME_MAX;
	ino_t ino, dino1, dino2, base_dino, root_ino = ROOT_INO;
	const char *base_dname = T_NAME;
	const char *dname1 = ut_randstr(ut_ctx, name_max);
	const char *dname2 = ut_randstr(ut_ctx, name_max);
	const char *name1 = NULL;
...
static void ut_rename_exchange(struct voluta_ut_ctx *ut_ctx)
{
	...
	ino_t ino1, ino2, dino1, dino2, root_ino = ROOT_INO;
	const char *dname1 = "dir1";
	const char *dname2 = "dir";
	const char *name1 = NULL;
	const char *name2 = NULL;
	const char *prefix = T_NAME;

	voluta_ut_mkdir_ok(ut_ctx, root_ino, dname1, &dino1);
	voluta_ut_mkdir_ok(ut_ctx, root_ino, dname2, &dino2);
	...
	voluta_ut_rmdir_ok(ut_ctx, root_ino, dname1);
	voluta_ut_rmdir_ok(ut_ctx, root_ino, dname2);
}
...
const struct voluta_ut_tests voluta_ut_utest_rename =
