...
static void ut_file_simple(struct voluta_ut_ctx *ut_ctx)
{
	...
	ut_file_simple_(ut_ctx, TERA - 111);
}
...
static void ut_file_tricky(struct voluta_ut_ctx *ut_ctx)
{
	...
	const loff_t mfact = (loff_t)FILEMAP_NCHILD;
	const loff_t moff1 = (loff_t)(BK_SIZE * FILEMAP_NCHILD);
	const loff_t moff2 = moff1 * mfact;
	const loff_t moff3 = (loff_t)FILESIZE_MAX / 2;
	const loff_t moff4 = (loff_t)FILESIZE_MAX - (loff_t)bsz;
...
static void ut_file_overwrite_complex(struct voluta_ut_ctx *ut_ctx)
{
	...
	ut_file_overwrite_complex_(ut_ctx, MEGA,
				   MEGA + BK_SIZE,
				   UMEGA);
	ut_file_overwrite_complex_(ut_ctx, MEGA - 7,
				   MEGA - 5,
				   (11 * BK_SIZE) + 11);
	ut_file_overwrite_complex_(ut_ctx, GIGA,
				   GIGA + BK_SIZE, UMEGA);
	ut_file_overwrite_complex_(ut_ctx, GIGA - 11111,
				   GIGA - 111, BK_SIZE + 11111);
	ut_file_overwrite_complex_(ut_ctx, TERA,
				   TERA + BK_SIZE, UMEGA);
...
static void ut_file_with_hole(struct voluta_ut_ctx *ut_ctx)
{
	...
	ut_file_with_hole_(ut_ctx, MEGA + 1,
			   MEGA + BK_SIZE + 2, BK_SIZE);
