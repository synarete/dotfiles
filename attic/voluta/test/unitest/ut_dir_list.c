...
dir_list(struct voluta_ut_ctx *ut_ctx, ino_t dino, size_t expected_nents)
{
	...
	while (keep_iter) {
		...
		keep_iter = (readdir_ctx->ndents > 0);
	}
...
static void dir_unlink_all(struct voluta_ut_dirlist *dl)
{
	...
	for (de = dl->list; de != NULL; de = de->next) {
		...
		name = de->dent.d_name;
		if (S_ISDIR(de->mode)) {
			voluta_ut_remove_dir(dl->ut_ctx, dl->dino, name);
			       ...
			       const char *dname, size_t count)
{
	...
	for (size_t i = 0; i < count; ++i) {
		name = make_name(ut_ctx, dname, i);
		if ((i % 3) == 0) {
			voluta_ut_make_dir(ut_ctx, dino, name, &ino);
		} else if ((i % 5) == 0) {
...
{
	ino_t dino, parent = VOLUTA_INO_ROOT;
	const char *dname = UT_NAME;
	struct voluta_ut_dirlist *dl;

	voluta_ut_make_dir(ut_ctx, parent, dname, &dino);
	...
	voluta_ut_remove_dir(ut_ctx, parent, dname);
}
...
const struct voluta_ut_tests voluta_ut_dir_list_tests = UT_MKTESTS(
			ut_local_tests);
