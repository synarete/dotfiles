...
{
	int err;
	ino_t ino, dino, parent = VOLUTA_INO_ROOT;
	struct stat st;
	const char *dname = "A";
...
static void ut_link_unlink_many(struct voluta_ut_ctx *ut_ctx)
{
	...
	for (i = 0; i < nlinks_max; i += 2) {
		lname = link_name(ut_ctx, fname, i + 1);
		voluta_ut_unlink_exists(ut_ctx, dino, lname);
	}
	for (i = 1; i < nlinks_max; i += 2) {
		lname = link_name(ut_ctx, fname, i + 1);
		voluta_ut_unlink_exists(ut_ctx, dino, lname);
	}
...
static void ut_inode_special(struct voluta_ut_ctx *ut_ctx)
{
	...
	voluta_ut_make_dir(ut_ctx, root_ino, dname, &dino);
	voluta_ut_create_special(ut_ctx, dino, fname, S_IFIFO | S_IRUSR, &ino);
	voluta_ut_remove_file(ut_ctx, dino, fname, ino);
	voluta_ut_create_special(ut_ctx, dino, fname, S_IFSOCK | S_IRUSR, &ino);
	voluta_ut_remove_file(ut_ctx, dino, fname, ino);
	voluta_ut_remove_dir(ut_ctx, root_ino, dname);
}
...
const struct voluta_ut_tests voluta_ut_namei_tests = UT_MKTESTS(ut_local_tests);
