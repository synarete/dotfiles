...
static struct voluta_ut_namesarr *
new_namesarr(struct voluta_ut_ctx *ut_ctx, size_t cnt)
{
	size_t sz;
	struct voluta_ut_namesarr *na;
...
{
	struct voluta_ut_namesarr *na = new_namesarr(ut_ctx, cnt);
...
static void ut_mkdir_subdirs_(struct voluta_ut_ctx *ut_ctx, size_t cnt)
{
	...
	struct voluta_ut_namesarr *na = make_names(ut_ctx, cnt);
...
static void ut_mkdir_multi_(struct voluta_ut_ctx *ut_ctx, size_t cnt)
{
	...
	const char *dname = T_NAME;
	const ino_t root_ino = ROOT_INO;
	struct voluta_ut_namesarr *na = make_names(ut_ctx, cnt + 1);
...
static void ut_mkdir_link_max(struct voluta_ut_ctx *ut_ctx)
{
	...
	struct voluta_ut_namesarr *na = make_names(ut_ctx, nlink_max);
...
static void ut_dir_link_any_names_(struct voluta_ut_ctx *ut_ctx, size_t cnt)
{
	...
	struct voluta_ut_namesarr *na = make_names_any_len(ut_ctx, cnt);
...
static void ut_dir_link_long_names_(struct voluta_ut_ctx *ut_ctx, size_t cnt)
{
	...
	const char *dname = T_NAME;
	const ino_t root_ino = ROOT_INO;
	struct voluta_ut_namesarr *na = make_long_names(ut_ctx, cnt);
...
static void ut_rmdir_when_open(struct voluta_ut_ctx *ut_ctx)
{
	ino_t ino;
	ino_t dino;
	ino_t parentd;
	...
	voluta_ut_mkdir_at_root(ut_ctx, name, &parentd);
	voluta_ut_mkdir_ok(ut_ctx, parentd, name, &dino);
	...
	voluta_ut_rmdir_ok(ut_ctx, parentd, name);
	voluta_ut_getattr_dir(ut_ctx, dino, &st);
	ut_assert_eq(st.st_nlink, 1);
	voluta_ut_create_noent(ut_ctx, dino, name);
	voluta_ut_releasedir_ok(ut_ctx, dino);
	voluta_ut_getattr_noent(ut_ctx, dino);
...
const struct voluta_ut_tests voluta_ut_test_dir =
