...
static struct ut_record *record_new(struct ut_env *ut_env,
				    size_t size)
{
	size_t rec_size;
	...
	rec = (struct ut_record *)ut_zerobuf(ut_env, sizeof(*rec));
	rec->data = ut_randbuf(ut_env, rec_size);
	rec->size = size;
...
static void ut_write_record(struct ut_env *ut_env, ino_t ino,
			    const struct ut_record *rec, loff_t off)
{
	const size_t bsz = record_size(rec, rec->size);

	ut_write_read(ut_env, ino, rec->data, bsz, off);
}

static void ut_read_record(struct ut_env *ut_env, ino_t ino,
			   const struct ut_record *rec, loff_t off)
{
	const size_t bsz = record_size(rec, rec->size);

	ut_read_ok(ut_env, ino, rec->data, bsz, off);
}

static void ut_read_record_verify(struct ut_env *ut_env, ino_t ino,
				  struct ut_record *rec, loff_t off)
{
	int err;

	ut_read_record(ut_env, ino, rec, off);
	record_decode(rec);
...
static void ut_file_records_seq_(struct ut_env *ut_env,
				 loff_t base, size_t size, size_t cnt)
{
	...
	for (size_t i = 0; i < cnt; ++i) {
		rec = record_new(ut_env, size);
		record_stamp_encode(rec, i);

		off = offset_of(rec, i, base);
		ut_write_record(ut_env, ino, rec, off);
	}
	for (size_t j = 0; j < cnt; ++j) {
		...
		ut_read_record_verify(ut_env, ino, rec, off);
	}
	ut_remove_file(ut_env, root_ino, name, ino);
}

static void ut_file_records_seq(struct ut_env *ut_env)
{
	loff_t base, base_offs[] = { 0, 111, 11111, 1111111, 111111111 };

	for (size_t i = 0; i < UT_ARRAY_SIZE(base_offs); ++i) {
		base = base_offs[i];
		ut_file_records_seq_(ut_env, base, 111, 11);
		ut_file_records_seq_(ut_env, base, 1111, 111);
		ut_file_records_seq_(ut_env, base, 11111, 1111);
	}
}
...
static void ut_file_records_rand_(struct ut_env *ut_env,
				  loff_t base, size_t size, size_t cnt)
{
	ino_t ino;
	loff_t off;
	...
	struct ut_record *rec;
	const char *name = UT_NAME;
	const long *pos = ut_randseq(ut_env, cnt, 0);
...
static void ut_file_records_rand_aligned(struct ut_env *ut_env)
{
	const loff_t base[] = { 0, UT_BK_SIZE, UT_UMEGA, UT_UGIGA, UT_UTERA };

	for (size_t i = 0; i < UT_ARRAY_SIZE(base); ++i) {
		ut_file_records_rand_(ut_env, base[i], UT_BK_SIZE, 512);
		ut_file_records_rand_(ut_env, base[i], 8 * UT_BK_SIZE, 64);
		ut_file_records_rand_(ut_env, base[i], UT_UMEGA, 8);
	}
}

static void ut_file_records_rand_unaligned1(struct ut_env *ut_env)
{
	const loff_t base[] = { 1, 111, 11111, 1111111, 111111111 };

	for (size_t i = 0; i < UT_ARRAY_SIZE(base); ++i) {
		ut_file_records_rand_(ut_env, base[i], 111, 1111);
		ut_file_records_rand_(ut_env, base[i], 1111, 111);
		ut_file_records_rand_(ut_env, base[i], 11111, 11);
	}
}

static void ut_file_records_rand_unaligned2(struct ut_env *ut_env)
{
	const loff_t base[] = {
	...
	for (size_t i = 0; i < UT_ARRAY_SIZE(base); ++i) {
		ut_file_records_rand_(ut_env, base[i], UT_BK_SIZE + 4, 64);
		ut_file_records_rand_(ut_env, base[i], 2 * UT_UMEGA, 8);
		ut_file_records_rand_(ut_env, base[i], size_max, 4);
	}
}
