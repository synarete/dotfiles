...
{
	int dfd, fd;
	size_t i, lim = VOLUTA_SYMLNK_MAX;
	loff_t off;
	size_t nch, nwr, nrd;
	mode_t ifmt = S_IFMT;
	struct stat st;
	...
	for (i = 1; i < lim; ++i) {
		...
		voluta_t_expect_true(S_ISLNK(st.st_mode));
		off = (loff_t)(i * lim);
		voluta_t_pwrite(fd, symval, i, off, &nwr);
		voluta_t_expect_eq(nwr, i);
	}
	for (i = 1; i < lim; ++i) {
		fill_name(name, sizeof(name), i);
		voluta_t_fstatat(dfd, name, &st, AT_SYMLINK_NOFOLLOW);
		...
		off = (loff_t)(i * lim);
		voluta_t_pread(fd, buf, i, off, &nrd);
		voluta_t_expect_eq(nrd, i);
...
voluta_t_test_symlink = VOLUTA_T_DEFTESTS(t_local_tests);
