...
static void test_sparse_simple(struct voluta_t_ctx *t_ctx)
{
	...
	for (size_t i = 0; i < cnt; ++i) {
		...
		pos = (loff_t)num;
		nsz = sizeof(num);
		voluta_t_pwriten(fd, &num, nsz, pos);
	}
	voluta_t_close(fd);
	...
	for (size_t j = 0; j < cnt; ++j) {
		...
		pos = (loff_t)num;
		nsz = sizeof(num2);
		voluta_t_preadn(fd, &num2, nsz, pos);
...
test_sparse_overwrite_(struct voluta_t_ctx *t_ctx, loff_t base_off)
{
	...
	const size_t noffs = ARRAY_SIZE(offs);
	...
	for (i = 0; i < noffs; ++i) {
		off = base_off + offs[i];
		voluta_t_pwriten(fd, buf1, len1, off);
	}
	for (i = 0; i < noffs; ++i) {
		off = base_off + offs[i] + 1;
		voluta_t_pwriten(fd, buf2, len2, off);
	}
	for (i = 0; i < noffs; ++i) {
		off = base_off + offs[i];
		voluta_t_preadn(fd, &byte, 1, off);
		voluta_t_expect_eq(buf1[0], byte);
		voluta_t_preadn(fd, buf3, len2, off + 1);
...
static void test_sparse_overwrite(struct voluta_t_ctx *t_ctx)
{
	...
	test_sparse_overwrite_(t_ctx, UMEGA - 2);
	test_sparse_overwrite_(t_ctx, UGIGA - 3);
}
