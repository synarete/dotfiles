...
{
	test_mmap_basic_(t_ctx, BK_SIZE, 3);
}

static void test_mmap_basic4(struct voluta_t_ctx *t_ctx)
{
	test_mmap_basic_(t_ctx, UMEGA, 4);
}
...
{
	test_mmap_after_write_(t_ctx, 0, UMEGA);
	test_mmap_after_write_(t_ctx, 0, 5 * UMEGA + 5);
	test_mmap_after_write_(t_ctx, UMEGA, UMEGA);
	test_mmap_after_write_(t_ctx, UMEGA, 7 * UMEGA + 7);
	test_mmap_after_write_(t_ctx, UGIGA, 11 * UMEGA + 11);
}

static void test_mmap_before_write(struct voluta_t_ctx *t_ctx)
{
	test_mmap_before_write_(t_ctx, 0, UMEGA);
	test_mmap_before_write_(t_ctx, 0, 5 * UMEGA + 5);
	test_mmap_before_write_(t_ctx, UMEGA, UMEGA);
	test_mmap_before_write_(t_ctx, UMEGA, 7 * UMEGA + 7);
	test_mmap_before_write_(t_ctx, UGIGA, 11 * UMEGA + 11);
}
...
{
	test_mmap_on_holes_(t_ctx, 0, UMEGA, 3);
	test_mmap_on_holes_(t_ctx, BK_SIZE + 5,
			    5 * BK_SIZE + 5, 5);
	test_mmap_on_holes_(t_ctx, 111 * BK_SIZE + 111, 11111, 111);
	test_mmap_on_holes_(t_ctx, UMEGA, 5 * UMEGA + 5, 5);
	test_mmap_on_holes_(t_ctx, UGIGA, 11 * UMEGA + 11, 11);
}
...
{
	int fd = -1;
	size_t nrd, bsz = BK_SIZE;
	loff_t off;
	void *addr;
	char *data, *buf1, *buf2;
	const size_t mlen = 4 * UMEGA;
	const int prot = PROT_READ | PROT_WRITE;
	const int flag = MAP_SHARED;
	const char *path = voluta_t_new_path_unique(t_ctx);
