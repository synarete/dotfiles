...
static void test_lseek_data_(struct voluta_t_ctx *t_ctx, size_t bsz)
{
	...
	uint8_t byte;
	uint8_t *buf1 = voluta_t_new_buf_rands(t_ctx, bsz);
	const char *path = voluta_t_new_path_unique(t_ctx);
	...
	voluta_t_readn(fd, &byte, 1);
	voluta_t_expect_eq(buf1[0], byte);
	voluta_t_preadn(fd, &byte, 1, pos + 1);
...
{
	test_lseek_data_(t_ctx, BK_SIZE);
	test_lseek_data_(t_ctx, 2 * BK_SIZE);
	test_lseek_data_(t_ctx, UMEGA);
}
...
{
	test_lseek_hole_(t_ctx, BK_SIZE);
	test_lseek_hole_(t_ctx, 2 * BK_SIZE);
	test_lseek_hole_(t_ctx, UMEGA);
}
