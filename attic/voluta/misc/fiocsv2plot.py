...
def fio_to_dot(fio_dat):
    return (fio_dat.wr_iops, fio_dat.wr_lat)


def fio_data_to_dots(fio_data):
    wr_iops = []
    wr_lat = []
    for fio_dat in fio_data:
...
def plot_fio_data(fio_data):
    mpl.style.use('bmh')
    fig, ax = plt.subplots()
    fig.set_facecolor('ghostwhite')
    fig.suptitle('Performance (random I/O)', fontsize='xx-large')
    ax.set_title('Performace as a function of CPUs (1,2,4,8,12,16,24,32)',
                 fontsize='x-large', pad=15)
    ax.set_xlabel('IOPS (4K randwrite)', fontsize='large', labelpad=15)
    ...
    for name in fio_data_dic:
        (wr_iops, wr_lat) = fio_data_to_dots(fio_data_dic[name])
