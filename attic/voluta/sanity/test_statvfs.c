...
static void test_statvfs_bfree_(struct voluta_t_ctx *t_ctx, loff_t off)
{
	...
	voluta_t_fstatvfs(fd, &stv1);
	voluta_t_pwrite(fd, "1", 1, off, &nwr);
...
static void test_statvfs_bfree(struct voluta_t_ctx *t_ctx)
{
	...
	test_statvfs_bfree_(t_ctx, VOLUTA_SMEGA);
	test_statvfs_bfree_(t_ctx, VOLUTA_SMEGA + 1);
	test_statvfs_bfree_(t_ctx, VOLUTA_STERA);
	test_statvfs_bfree_(t_ctx, VOLUTA_FILESIZE_MAX - 1);
}
