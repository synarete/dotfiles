...
test_sequencial_aligned_blk(struct voluta_t_ctx *t_ctx, size_t cnt)
{
	...
	from = (loff_t)(VOLUTA_GIGA + VOLUTA_BK_SIZE);
	test_sequencial_io(t_ctx, from, bsz, cnt);
	from = (loff_t)(VOLUTA_GIGA - (bsz * cnt));
	test_sequencial_io(t_ctx, from, bsz, cnt);
	from = (loff_t)((VOLUTA_GIGA * 2) / 2);
	test_sequencial_io(t_ctx, from, bsz, cnt);
				      ...
				      size_t n)
{
	...
	test_sequencial_nstrings(t_ctx, VOLUTA_BK_SIZE - 1, n);
	test_sequencial_nstrings(t_ctx, VOLUTA_BK_SIZE, n);
	test_sequencial_nstrings(t_ctx, VOLUTA_MEGA - 1, n);
	test_sequencial_nstrings(t_ctx, VOLUTA_GIGA, n);
}
