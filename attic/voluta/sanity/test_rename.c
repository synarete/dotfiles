 ...
 * target.
 */
static void test_rename_override_(struct voluta_t_ctx *t_ctx, size_t cnt)
{
	int fd;
	void *buf;
	size_t i, nwr, bsz = VOLUTA_BK_SIZE;
	char *src_path1, *tgt_path1;
	const char *src_path0 = voluta_t_new_path_unique(t_ctx);
	const char *tgt_path0 = voluta_t_new_path_unique(t_ctx);
	...
	for (i = 0; i < cnt; ++i) {
		...
		voluta_t_pwrite(fd, buf, bsz, (loff_t)i, &nwr);
		voluta_t_close(fd);
	}
	for (i = 0; i < cnt; ++i) {
		src_path1 = voluta_t_new_pathf(t_ctx, src_path0, "s%08x", i);
...
{
	test_rename_override_(t_ctx, 1);
	test_rename_override_(t_ctx, 2);
	test_rename_override_(t_ctx, 3);
	test_rename_override_(t_ctx, 1024);
}
...
static void test_renameat_inplace_rw(struct voluta_t_ctx *t_ctx)
{
	...
	const char *name1;
	const char *name2;
	const size_t bsz = 64 * VOLUTA_KILO;
	const char ch = 'B';
	const char *path = voluta_t_new_path_unique(t_ctx);
