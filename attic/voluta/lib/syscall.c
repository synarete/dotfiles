...
/* PROC */
/*
 * TODO-0019: Move to 'readfile' API
 */
...
static int sys_readproc_value(const char *procdir, const char *filename,
			      long *out_value)
{
	int err;
	size_t nrd = 0;
...
{
	return sys_readproc_value("/proc/sys/fs", "pipe-max-size", out_value);
}
